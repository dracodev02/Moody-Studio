...
const OurProject = () => {
  ...
  const isInView = useInView(ourProjectRef, { amount: 0.3, once: true });
  return (
    <div
      id="projects"
      className="max-w-[1232px] mx-auto p-10 max-md:p-2 overflow-hidden rounded-3xl"
    >
      <div ref={ourProjectRef} className={`rounded-3xl overflow-hidden shadow-[0px_0px_72px_rgba(146,7,7,0.5)] py-10 relative border border-primary`}>
        <img
          src="/our-project.png"
          className="h-full w-full absolute top-0 left-0 -z-[1] brightness-[25%]"
          alt=""
        />
        <div className={`relative  ${isInView ? "animate-fade-up opacity-100" : "opacity-0"}`}>
        ...
        <div className={`flex gap-10 max-sm:gap-5 items-center justify-center mt-[76px] py-10  ${isInView ? "animate-fade-up opacity-100" : "opacity-0"}`}>
